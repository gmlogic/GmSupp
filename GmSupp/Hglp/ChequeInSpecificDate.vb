...
Public Class ChequeInSpecificDate
...
#Region "03-Load Form"
    Private Sub MyBase_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        DateTimePickerLimit.Value = DateTimePicker2.Value 'CDate("31/12/" & Year(CTODate) - 1)
        ...
        'If {"dmalandrakis", "iantypa", "kvasilaki", "afarasoglou", "mourailidou"}.Contains(CurUser) Then
        ...
        If CurUser = "gmlogic" Then
            ...
            'Me.radioBtnOI.Enabled = True
            'Me.TlSTxtWHOUSE.Text = "4,13,8,5"
            'Me.TlSTxtTRDR.Text = "3000003" '"3001030" '"3000533" '"3000011" '"3000382" ' '"3000934" '"3000534" '"3000011" ' "3000153" '"5001104,5000386"
            Me.txtBoxChequeCODE.Text = "ΕΠΕΠ000056"
            'Me.radioBtnOI.Checked = True
            ...
            Me.DateTimePicker1.Value = CDate("01/06/2022")
            Me.DateTimePicker2.Value = New Date(CTODate.Year, CTODate.Month, CTODate.Day, 23, 59, 59)
...
#Region "04-Bas_Commands"
    ...
    Private Sub Cmd_Select()
        ...
        Try
            ...
            Me.Tag = "Aξιογράφα σε συγκεκριμένη ημερομηνία"
            If Me.Tag = "Aξιογράφα σε συγκεκριμένη ημερομηνία" Then ' "Πελατών" Then

                'Αξιόγραφα
                If Me.radioBtnCheque.Checked Then
                    Dim q = db.ccCVChequeOpens.Where(Function(f) f.CRTDATE >= DateTimePicker1.Value.Date And f.CRTDATE <= Me.DateTimePicker2.Value).ToList

                    Dim qwh = db.CHEQUEs.Where(Function(f) f.CRTDATE >= DateTimePicker1.Value.Date And f.CRTDATE <= Me.DateTimePicker2.Value).ToList ' trdrs.Contains(f.TRDR)).ToList ' db.CHEQUEs.Where(Function(f) trdrs.Contains(f.TRDR)).ToList '.AsQueryable
                    qwh = qwh.OrderBy(Function(f) f.CODE).ThenBy(Function(f) f.CHEQUE).ToList
                    Dim dateName = "CRTDATE"
                    'If Not Me.radioBtnFinalDate.Checked Then

                    'qwh = qwh.Where(Function(f) f.CRTDATE >= DateTimePicker1.Value.Date And f.CRTDATE <= Me.DateTimePicker2.Value).ToList '.AsQueryable

                    'If ToolStripComboBox1.SelectedItem = "Εκδότης:" Then
                    '    qwh = qwh.Where(Function(f) trdrs.Contains(f.TRDRPUBLISHER)).ToList
                    'End If

                    'If ToolStripComboBox1.SelectedItem = "Αρχικός συναλ/νος:" Then
                    '    qwh = qwh.Where(Function(f) trdrs.Contains(f.TRDR)).ToList
                    'End If
                    'Else
                    '    dateName = "FINALDATE"
                    ...
                    If Not Me.txtBoxChequeCODE.Text = "" Then
                        qwh = qwh.Where(Function(f) Me.txtBoxChequeCODE.Text.Replace("*", "").Split(";").Contains(f.CODE)).ToList 'Σειρά
                    End If

                    If Not Me.txtBoxChequeNUMBER.Text = "" Then
                        qwh = qwh.Where(Function(f) Me.txtBoxChequeNUMBER.Text.Replace("*", "").Split(";").Contains(f.CHEQUENUMBER)).ToList 'Σειρά
                    End If


                    Dim fnchs As New List(Of Hglp.fnCheQueOpenAllResult)
                    ...
                    For Each chq In qwh
                        ...
                        '    fnchs.Add(db.fnCheQueOpenAll(chq.CHEQUE, Me.DateTimePickerLimit.Value).OrderBy(Function(f) f.TRNDATE).ThenBy(Function(f) f.FINDOC).ToList.LastOrDefault)
                        'End If
                        'If Me.radioBtnAnalytical.Checked Then
                        Dim fnch As New List(Of Hglp.fnCheQueOpenAllResult)
                            Try
                                ...
                                'fnch = (From tf In db.TRDFLINEs Join fi In db.FINDOCs On tf.FINDOC Equals fi.FINDOC
                                '        Where tf.CHEQUE = chq.CHEQUE And fi.TRNDATE < Me.DateTimePickerLimit.Value.AddDays(1) And fi.APPRV = 1 And fi.ISCANCEL = 0
                                '        Select New fnCheQueOpenAllResult With {.TRNDATE = fi.TRNDATE, .CHEQUE = tf.CHEQUE, .FINCODE = fi.FINCODE}).ToList
                                'If fnch.Count = 0 Then
                                '    Continue For
                                'End If
                                fnch = db.fnCheQueOpenAll(chq.CHEQUE, Me.DateTimePickerLimit.Value).ToList
                                fnch = fnch.OrderBy(Function(f) f.TRNDATE).ThenBy(Function(f) f.FINDOC).ToList
                                If fnch.Count > 0 Then
                                'fnch = (From fn In fnch Join ch In db.CHEQUEs On fn.CHEQUE Equals ch.CHEQUE
                                '        Order By fn.TRNDATE
                                '        Select New fnCheQueOpenAllResult With {.CODE = ch.CODE, .CHEQUE = ch.CHEQUE, .FINCODE = fn.FINCODE}).ToList
                                ''fnch = fnch.OrderBy(Function(f) f.TRNDATE).ToList
                                ...
                                Dim LDate As Date
                                Dim BFDate As Date
                                Dim BFDCHEQUEUPDNAME As String = ""
                                Dim LDCHQTPRMSNAME As String = ""
                                If fnch.Count >= 2 Then
                                    For i = 0 To fnch.Count - 1
                                        If i = fnch.Count - 2 Then
                                            BFDate = fnch(i).TRNDATE
                                            BFDCHEQUEUPDNAME = fnch(i).CHQTPRMSNAME
                                        End If
                                        If i = fnch.Count - 1 Then
                                            LDate = fnch(i).TRNDATE
                                            LDCHQTPRMSNAME = fnch(i).CHQTPRMSNAME
                                        End If
                                    Next
                                End If
                                If Me.radioBtnΑggregate.Checked Then
                                    If fnch.Count = 1 Then
                                        fnch.FirstOrDefault.BFDATE = Nothing
                                        fnch.FirstOrDefault.CHEQUEUPDNAME = Nothing
                                        fnch.FirstOrDefault.LDATE = fnch(0).TRNDATE
                                        fnch.FirstOrDefault.CHQTPRMSNAME = fnch(0).CHQTPRMSNAME
                                        fnch.FirstOrDefault.LBFDIFFDATE = Nothing ' LDate.Subtract(BFDate).Days
                                    Else
                                        fnch.FirstOrDefault.BFDATE = BFDate
                                        fnch.FirstOrDefault.CHEQUEUPDNAME = BFDCHEQUEUPDNAME
                                        fnch.FirstOrDefault.LDATE = LDate
                                        fnch.FirstOrDefault.CHQTPRMSNAME = LDCHQTPRMSNAME
                                        fnch.FirstOrDefault.LBFDIFFDATE = LDate.Subtract(BFDate).Days
                                    End If
                                End If
                            Else
                                Dim aa = 1
                    ...
                    If Me.radioBtnΑggregate.Checked Then
                        ...
                        'myArrF = ("CODE,SERIESNUM,BNKNAME,CRTDATE,FINALDATE,TDCODE,TDBCODE,TDBNAME,CREATORNAME,CHEQUESTATESNAME,CHEQUEBAL,TRNSTATENAME,TRNBALANCE,POSSTYPE,TRNHOLDERNAME,CHEQUENUMBER,CHEQUEVAL,BFDATE,CHEQUEUPDNAME,LDATE,CHQTPRMSNAME,LBFDIFFDATE,CHEQUE").Split(",")
                        'myArrN = ("Κωδικός,Αριθμός,Τράπεζα,Παραλαβή,Λήξη,Αρχικός συναλ/νος,Κωδικός εκδοτη,Επωνυμία εκδοτη,Όνομα,Κατάσταση Τρέχουσα,Υπόλοιπο Τρέχων,Κατάσταση στην Οριακή Ημ/νία,Υπόλοιπο στήν Οριακή Ημ/νία,Κατηγορία κάτοχου,Όνομα κάτοχου,Αριθμός,Ποσό,Προτελευταία κίνηση Ημ/νία,Προτελευταία κίνηση,Τελευταία κίνηση Ημ/νία,Τελευταία κίνηση,Ημέρες,CHEQUE").Split(",")

                        'myArrF = ("CODE,SERIESNUM,BNKNAME,CRTDATE,FINALDATE,TDCODE,TDBCODE,TDBNAME,CREATORNAME,CHEQUESTATESNAME,CHEQUEBAL,TRNSTATENAME,TRNBALANCE,POSSTYPE,TRNHOLDERNAME,CHEQUENUMBER,CHEQUEVAL,TRNDATE,FINCODE,CHQTPRMSNAME,LINEVAL,CHEQUEUPD,CHEQUEUPDNAME,TRDFLINESCHEQUESTATESNAME,CHEQUE,FINDOC,TRDR,TRNSTATE,TRNTYPEPOSS,TRNFOLDER,OPENBAL,PAYBAL").Split(",")
                        'myArrN = ("Κωδικός,Αριθμός,Τράπεζα,Παραλαβή,Λήξη,Αρχικός συναλ/νος,Κωδικός εκδοτη,Επωνυμία εκδοτη,Όνομα,Κατάσταση Τρέχουσα,Υπόλοιπο Τρέχων,Κατάσταση στην Οριακή Ημ/νία,Υπόλοιπο στήν Οριακή Ημ/νία,Κατηγορία κάτοχου,Όνομα κάτοχου,Αριθμός,Ποσό,Ημερ/νία,Παραστατικό,Κίνηση,Αξία,CHEQUEUPD,Ενημέρωση,Νέα κατάσταση,CHEQUE,FINDOC,TRDR,TRNSTATE,TRNTYPEPOSS,TRNFOLDER,OPENBAL,PAYBAL").Split(",")

                        fnchs = (From fn In fnchs Group By fn.CHEQUE Into Group
                                 Select New Hglp.fnCheQueOpenAllResult With {
                                     .CODE = Group.FirstOrDefault.CODE,
                                     .SERIESNUM = Group.FirstOrDefault.SERIESNUM,
                                     .BNKNAME = Group.FirstOrDefault.BNKNAME,
                                     .CRTDATE = Group.FirstOrDefault.CRTDATE,
                                     .FINALDATE = Group.FirstOrDefault.FINALDATE,
                                     .TDCODE = Group.FirstOrDefault.TDCODE,
                                     .TDBCODE = Group.FirstOrDefault.TDBCODE,
                                     .TDBNAME = Group.FirstOrDefault.TDBNAME,
                                     .CREATORNAME = Group.FirstOrDefault.CREATORNAME,
                                     .CHEQUESTATESNAME = Group.FirstOrDefault.CHEQUESTATESNAME,
                                     .CHEQUEBAL = Group.FirstOrDefault.CHEQUEBAL,
                                     .TRNSTATENAME = Group.FirstOrDefault.TRNSTATENAME,
                                     .TRNBALANCE = Group.FirstOrDefault.TRNBALANCE,
                                     .POSSTYPE = Group.FirstOrDefault.POSSTYPE,
                                     .TRNHOLDERNAME = Group.FirstOrDefault.TRNHOLDERNAME,
                                     .CHEQUENUMBER = Group.FirstOrDefault.CHEQUENUMBER,
                                     .CHEQUEVAL = Group.FirstOrDefault.CHEQUEVAL,
                                     .CHEQUE = Group.FirstOrDefault.CHEQUE,
                                     .BFDATE = Group.FirstOrDefault.BFDATE,
                                     .CHEQUEUPDNAME = Group.FirstOrDefault.CHEQUEUPDNAME,
                                     .LDATE = Group.FirstOrDefault.LDATE,
                                     .CHQTPRMSNAME = Group.FirstOrDefault.CHQTPRMSNAME,
                                     .LBFDIFFDATE = Group.FirstOrDefault.LBFDIFFDATE
                                     }).ToList
                    ...
                    Me.MasterBindingSource.DataSource = New SortableBindingList(Of Hglp.fnCheQueOpenAllResult)(fnchs.ToList)
                ...
                '    Dim detb = False
                '    Dim detailsAll As New List(Of GetTrdrDetailResult)
                '    Dim curBals As New List(Of GetTrdrBalanceAllResult)
                '    Dim trdrtbl As New List(Of TRDR)
                '    If Me.radioBtnCalcDetails.Checked Then
                '        detb = True
                '        'detailsAll = GetTrdrDetailBalance(curBals, prsns, Nothing, trdrCodes.Replace("*", "%").Trim, 1000, sodTyp, Me.DateTimePicker1.Value, Me.DateTimePicker2.Value, True)
                '    End If
                ...
                '    'myArrN = ("Ημερ/νία,Παραστατικό,Κίνηση,Α.Χ.,Επωνυμία,Ποσ.1,Ποσ.εισαγωγών,Αξία εισαγωγών,Ποσ.εξαγωγών,Αξία εξαγωγών,Υπόλοιπο ποσ.,Υπόλοιπο αξία,Αιτιολογία").Split(",")
                '    detailsAll = res.GetResult(Of GetTrdrDetailResult).ToList
                ...
                '        Dim detailsTd = detailsAll.Where(Function(f) f.trdr = td).ToList
                '        Dim trdr = trdrtbl.Where(Function(f) f.TRDR = td).FirstOrDefault
                '        Dim trdExtra = trdExtratbl.Where(Function(f) f.TRDR = td).FirstOrDefault
                '        Dim SALNAME = ""
                ...
                '        Dim fins = finss.Where(Function(f) f.TRDR = td And f.OITYPE = 1).ToList
                ...
                '        '    bal = trfin.LBAL
                '        'End If
                '        If Not IsNothing(curBal) Then
                '            bal = curBal.FirstOrDefault.pbal
                '        End If
                ...
                '        detNew.CRDLIMIT1 = trdr.CRDLIMIT1
                '        detNew.CRDLIMIT2 = trdr.CRDLIMIT2
                '        detNew.ExNum01 = trdExtra.NUM01
                ...
                '        'Υπολογισμός με κινήσεις καρτέλας (Από-Έως ημ/νία)
                '        If Me.radioBtnCalcDetails.Checked Then
                '            Dim dets = detailsTd.Where(Function(f) f.trdr = td).ToList
                ...
                '                det.afm = trdr.AFM
                '                det.tdSalesManName = SALNAME
                '                det.CRDLIMIT1 = trdr.CRDLIMIT1
                '                det.CRDLIMIT2 = trdr.CRDLIMIT2
                '                det.ExNum01 = trdExtra.NUM01
                '                If {1351}.Contains(det.sosource) And det.tfprms = 102 And Not {9961}.Contains(det.fprms) Then
                '                    Dim tdebit As Double = det.tdebit
                '                    det.TVAL = det.tdebit
                '                    det.STAMNT = det.tdebit
                ...
                '                            det.OPNTVAL = Math.Abs(tdet.tdebit.Value)
                '                            tdebit += tdet.tdebit
                '                        End If
                '                    End If
                '                    det.SOPNTAMNT = tdebit
                '                    'FP.PAYDEMANDMD AS OITYPE
                '                    'OITYPE = 1 Δημιουργία
                '                    'OITYPE = 2 Κάλυψη
                '                    det.OITYPE = 1
                '                    detailsTd.Add(det)
                ...
                '            Next
                '        Else
                '            'fins = fins.Where(Function(f) f.TRNDATE >= DateTimePicker1.Value And f.TRNDATE <= DateTimePicker2.Value).ToList
                ...
                '    'Μόνο Ανεξόφλητα and Αναλυτικά-Κινήσεις '''''and Υπολογισμός βάση καρτέλας
                '    If Me.chkBoxoiBal.Checked And Me.radioBtnAnalytical.Checked Then ' And Me.radioBtnCalcDetails.Checked Then
                '        If Me.radioBtnCalcBal.Checked Then 'Ανεξόφλητα με - Ανοιχτό Υπόλοιπο
                '            details = details.Where(Function(f) Not f.oiBal = 0).ToList
                '        End If
                '        If Me.radioBtnCalcMixBal.Checked Then 'Ανεξόφλητα με - Μικτό Υπόλοιπο
                ...
                '                 Group g1 By g1.trdr, g1.code, g1.name, g1.tdSalesManName, g1.afm, g1.ExNum01, g1.CRDLIMIT1, g1.CRDLIMIT2 Into Group).ToList
                ...
                '            r4.afm = q1.afm
                '            r4.CRDLIMIT1 = q1.CRDLIMIT1
                '            r4.CRDLIMIT2 = q1.CRDLIMIT2
                '            r4.ExNum01 = q1.ExNum01
                ...
                '            'Εκ μεταφοράς < της τρέχουσας χρήσης - Τρέχουσα χρήση = (Me.DateTimePicker2.Value.Year)
                '            r4.oibal00 = Math.Round(q1.Group.Where(Function(f) f.trndate.Year < Me.DateTimePicker2.Value.Year).Sum(Function(f) f.oiBal), 2, MidpointRounding.AwayFromZero)
                '            r4.oibal13 = r4.oibal00 'Ανεξώφλητη αξία από προηγ.Χρήση

                '            For i = 1 To 12
                '                Dim month = i
                '                'Τρέχουσα χρήση = (Me.DateTimePicker2.Value.Year)
                '                Dim monthVal = Math.Round(q1.Group.Where(Function(f) f.trndate.Year = Me.DateTimePicker2.Value.Year And f.trndate.Month = month And Not f.oiBal = 0).Sum(Function(f) f.oiBal), 2, MidpointRounding.AwayFromZero)
                ...
                '                r4.CRDLIMIT2 = trdr.CRDLIMIT2 'ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 2
                ...
                '        Me.lblH08.Text = "Ανεξόφλητα Αξιόγραφα"
                '        If Me.radioBtnCalcBal.Checked Then
                '            Me.lblH09.Text = "Ανεξόφλητα"
                '        End If
                '        If Me.radioBtnCalcMixBal.Checked Then
                '            Me.lblH09.Text = "Μικτό Ανεξόφλητα"
                '        End If
                ...
                '            Dim lastoChkQue As Decimal = details.LastOrDefault.oChkQue
                ...
                '            Me.lblV08.Text = String.Format("{0:N2}", lastoChkQue)
                '            If Me.radioBtnCalcBal.Checked Then
                '                Me.lblV09.Text = String.Format("{0:N2}", details.Sum(Function(f) f.oiBal))
                '            End If
                '            If Me.radioBtnCalcMixBal.Checked Then
                '                Me.lblV09.Text = String.Format("{0:N2}", details.Sum(Function(f) f.oiMixBal))
                '            End If
                '        End If
                '    End If
...
#Region "96-MasterDataGridView"
    ...
    Private Sub MasterDataGridView_Styling()
        Try
            ...
            End If
            If Me.Tag = "Aξιογράφα σε συγκεκριμένη ημερομηνία" Then
                ...
                '    If Me.radioBtnCalcBal.Checked Then
                '        myArrF = ("rowNo,fincode,code,name,afm,ExNum01,CRDLIMIT1,CRDLIMIT2,tdSalesManName,trndate,tdebit,tcredit,bal,oiBal,oDays,oChkQue,oBalMax,oBalDate,tturnovr,Vat,findoc").Split(",")
                '        myArrN = ("rowNo,Παρ/κό,ΚΩΔ,ΣΥΝΑΛΑΣΟΜΕΝΟΣ,ΑΦΜ,ΑΣΦΑΛΙΣΤΙΚΟ ΟΡΙΟ,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 1,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 2,Πωλητής,ΗΜ/ΝΙΑ,Αξία παρ/κου,Πίστωση,Ανοιχτό Υπόλοιπο,Ανεξόφλητα,oDays,Ανοιχτά Αξιόγραφα,OpenBalMax,oBalDate,Τζίρος,ΦΠΑ,findoc").Split(",")
                '    End If

                '    If Me.radioBtnCalcMixBal.Checked Then
                '        myArrF = ("rowNo,fincode,code,name,afm,tdSalesManName,trndate,tdebit,tcredit,bal,mixBal,oiMixBal,oDays,oChkQue,oBalMax,oBalDate,tturnovr,Vat,findoc").Split(",")
                '        myArrN = ("rowNo,Παρ/κό,ΚΩΔ,ΣΥΝΑΛΑΣΟΜΕΝΟΣ,ΑΦΜ,Πωλητής,ΗΜ/ΝΙΑ,Αξία παρ/κου,Πίστωση,Ανοιχτό Υπόλοιπο,Μικτό υπόλοιπο,Μικτό Ανεξόφλητα,oDays,Ανοιχτά Αξιόγραφα,OpenBalMax,oBalDate,Τζίρος,ΦΠΑ,findoc").Split(",")
                '    End If
                ...
                'End If
                'If Me.radioBtnΑggregate.Checked And Me.radioBtnOI.Checked Then
                '    myArrF = ("tdSalesManName,code,name,afm,ExNum01,CRDLIMIT1,CRDLIMIT2,oibal00,oibal01,oibal02,oibal03,oibal04,oibal05,oibal06,oibal07,oibal08,oibal09,oibal10,oibal11,oibal12,oibal13").Split(",")
                '    myArrN = ("Πωλητής,ΚΩΔ,ΣΥΝΑΛΑΣΟΜΕΝΟΣ,ΑΦΜ,ΑΣΦΑΛΙΣΤΙΚΟ ΟΡΙΟ,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 1,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 2,ΕκΜεταφ,Ιανουάριος,Φεβρουάριος,Μάρτιος,Απρίλιος,Μάιος,Ιούνιος,Ιούλιος,Αύγουστος,Σεπτέμβριος,Οκτώβριος,Νοέμβριος,Δεκέμβριος,ΣυνΥπολ").Split(",")
                '    If Me.chkBoxTest.Checked Then
                ...
                '    myArrF = ("tdSalesManName,code,name,afm,ExNum01,CRDLIMIT1,CRDLIMIT2,tdebit,tcredit,bal,oiBal,oiMixBal,oChkQue,mixBal,oBalMax,oBalDate,tturnovr,Vat").Split(",")
                '    myArrN = ("Πωλητής,ΚΩΔ,ΣΥΝΑΛΑΣΟΜΕΝΟΣ,ΑΦΜ,ΑΣΦΑΛΙΣΤΙΚΟ ΟΡΙΟ,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 1,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 2,Αξία παρ/κου,Πίστωση,Ανοιχτό Υπόλοιπο,Ανεξόφλητα,Μικτό Ανεξόφλητα,Ανοιχτά Αξιόγραφα,Μικτό υπόλοιπο,OpenBalMax,oBalDate,Τζίρος,ΦΠΑ").Split(",")
                'End If
                ...
                '        myArrF = ("InsuranceChoice,code,name,afm,ExNum01,ExDate01,CRDLIMIT1,CRDLIMIT2,ExDate02,ExNum02,bal,mxe,mixBal,InsuranceLimitException,CreditLimitException,CreditCustomerException,BalMoreThan181").Split(",")
                '        myArrN = ("ΧΑΡΑΚΤΗΡΙΣΜΟΣ ΑΣΦΑΛΙΣΗΣ,ΚΩΔΙΚΟΣ,ΕΠΩΝΥΜΙΑ ΠΕΛΑΤΗ,ΑΦΜ,ΑΣΦΑΛΙΣΤΙΚΟ ΟΡΙΟ,ΛΗΞΗ ΑΣΦΑΛΙΣΤΙΚΟΥ ΟΡΙΟΥ,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 1,ΙΣΤΩΤΙΚΟ ΟΡΙΟ 2,ΛΗΞΗ ΠΙΣΤ ΟΡΙΟΥ 1,ΗΜΕΡΕΣ ΠΙΣΤΩΣΗΣ,ΑΝΟΙΚΤΟ ΥΠΟΛΟΙΠΟ,ΜΧΕ,ΜΕΙΚΤΟ ΥΠΟΛΟΙΠΟ,ΥΠΕΡΒΑΣΗ ΑΣΦΑΛΙΣΤΙΚΟΥ ΟΡΙΟΥ,ΥΠΕΡΒΑΣΗ ΠΙΣΤΩΤΙΚΟΥ ΟΡΙΟΥ,ΥΠΕΡΒΑΣΗ ΠΙΣΤΩΣΗΣ ΠΕΛΑΤΗ,ΥΠΕΡΒΑΣΗ ΠΙΣΤΩΣΗΣ > 180 ΗΜΕΡΩΝ").Split(",")
                '    End If
                ...
                '        myArrF = ("InsuranceChoice,code,name,afm,ExNum01,ExDate01,CRDLIMIT1,CRDLIMIT2,ExDate02,ExNum02,bal,mxe,mixBal,Bal001_030,Bal031_060,Bal061_090,Bal091_120,Bal121_150,Bal151_180,BalMoreThan181").Split(",")
                '        myArrN = ("ΧΑΡΑΚΤΗΡΙΣΜΟΣ ΑΣΦΑΛΙΣΗΣ,ΚΩΔΙΚΟΣ,ΕΠΩΝΥΜΙΑ ΠΕΛΑΤΗ,ΑΦΜ,ΑΣΦΑΛΙΣΤΙΚΟ ΟΡΙΟ,ΛΗΞΗ ΑΣΦΑΛΙΣΤΙΚΟΥ ΟΡΙΟΥ,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 1,ΠΙΣΤΩΤΙΚΟ ΟΡΙΟ 2,ΛΗΞΗ ΠΙΣΤ ΟΡΙΟΥ 1,ΗΜΕΡΕΣ ΠΙΣΤΩΣΗΣ,ΑΝΟΙΚΤΟ ΥΠΟΛΟΙΠΟ,ΜΧΕ,ΜΕΙΚΤΟ ΥΠΟΛΟΙΠΟ,ΥΠΟΛΟΙΠΟ 1-30 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ 31-60 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ 61-90 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ 91-120 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ 121-150 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ 151-180 ΗΜΕΡΕΣ,ΥΠΟΛΟΙΠΟ > 181 ΗΜΕΡΕΣ").Split(",")
                        ...
                        With col
                            If t.IsGenericType AndAlso t.GetGenericTypeDefinition = GetType(Nullable(Of )) Then
                                ...
                                If Not t.FullName.IndexOf("System.DateTime, ") = -1 Or Not t.FullName.IndexOf("System.Decimal") = -1 Or Not t.FullName.IndexOf("System.Money") = -1 Then
                                    If {"TRNDATE", "BFDATE", "LDATE"}.Contains(col.DataPropertyName) Then
...
#Region "97- Control Events"
    ...
    Private Sub TlSBtn_Click(sender As Object, e As EventArgs) Handles TlSBtnWHOUSE.Click, TlSBtnTRDR.Click
    ...
    Private Sub TlSTextBox_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        Try
            ...
            'rem m_dtGen = GmData.GetTableSQL(conn, CommandType.Text, mSql, , RsTables)
        Catch ex As Exception
    ...
    'Private Sub radioBtn_CheckedChanged(sender As Object, e As EventArgs)
        ...
        Using p As New ExcelPackage()
            Try
                ...
                For Each ws As ExcelWorksheet In p.Workbook.Worksheets
                    ...
                    cellH.Value = Me.Text & " - " & Me.DateTimePickerLimit.Value.ToString
...
Friend Class monthsBal1
    ...
    Public Property ExNum01 As Double?
    Public Property CRDLIMIT1 As Double?
    Public Property CRDLIMIT2 As Double?
End Class
Friend Class monthsSales1
    ''' <summary>
    ''' 
    ''' </summary>
    ''' <returns></returns>
    ...
    Public Property LDebit As Decimal
    Public Property LTurnOvr As Decimal
    Public Property trdr As Integer

End Class
Friend Class carierPmnt1
