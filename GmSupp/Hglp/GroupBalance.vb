...
#Region "03-Load Form"
    Private Sub MyBase_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        If CurUser = "gmlogic" Then
            ...
            Me.DateTimePicker1.Value = CDate("01/01/2020")
            Me.DateTimePicker2.Value = CDate("31/01/2020") ' CTODate ' CDate("31/03/2018")
    ...
    Private Sub Cmd_Select()
        ...
        Try
            ...
            If Me.Tag = "Πελατών" Then
                ...
                Dim trdrs As New List(Of Integer)
                Dim prsns As New List(Of PRSN)
                Dim trdrCodes = ""
                If Not Me.TlSTxtTRDR.Text = "" Then
                    trdrCodes = Me.TlSTxtTRDR.Text
                End If

                If trdrCodes.Contains("*") Then
                    trdrs = db.TRDRs.Where(Function(f) f.SODTYPE = sodTyp And f.CODE Like trdrCodes).Select(Function(f) f.TRDR).ToList
                ElseIf trdrCodes.Contains(",") Then
                    trdrs = db.TRDRs.Where(Function(f) trdrCodes.Replace("*", "").Split(",").Contains(f.CODE)).Select(Function(f) f.TRDR).ToList
                ElseIf Not Me.TlSTxtPRSN.Text = "" Then
                    prsns = db.PRSNs.Where(Function(f) f.SODTYPE = 20).ToList
                    ...
                    trdrs = db.TRDRs.Where(Function(f) f.SODTYPE = sodTyp And prsnids.Contains(f.SALESMAN)).Select(Function(f) f.TRDR).ToList
                    Dim codes = db.TRDRs.Where(Function(f) trdrs.Contains(f.TRDR)).Select(Function(f) f.CODE).ToList

                    For Each ss In codes
                        trdrCodes &= "," & ss
                    Next
                Else
                    trdrs = db.TRDRs.Where(Function(f) f.SODTYPE = sodTyp).Select(Function(f) f.TRDR).ToList
                End If
                ...
                If trdrCodes = "*" Then
                    trdrs = db.TRDRs.Where(Function(f) f.SODTYPE = sodTyp).Select(Function(f) f.TRDR).ToList
                Else
                    trdrs = db.TRDRs.Where(Function(f) trdrCodes.Split(",").Contains(f.CODE)).Select(Function(f) f.TRDR).ToList
                End If
                ...
                If Me.radioBtnSales.Checked Then
                    ...
                    Dim trdbs As New List(Of TRDBALSHEET)
                    If trdrs.Count < 2100 Then
                        trdbs = db.TRDBALSHEETs.Where(Function(f) trdrs.Contains(f.TRDR) And months.Select(Function(f1) f1.FiscPrd).Distinct.Contains(f.FISCPRD)).ToList
                    Else
                        For Each tr In trdrs
                            Dim tb = db.TRDBALSHEETs.Where(Function(f) f.TRDR = tr).FirstOrDefault
                            If Not IsNothing(tb) Then
                                trdbs.Add(tb)
                            End If
                        Next
                        trdbs = trdbs.Where(Function(f) months.Select(Function(f1) f1.FiscPrd).Distinct.Contains(f.FISCPRD)).ToList
                    End If
                    ...
                    dt.Columns.Add("CRDLIMIT1", GetType(System.Double))
                    dt.Columns.Item(dt.Columns.IndexOf("CRDLIMIT1")).Caption = "Πιστ/κό όριο 1"
                    dt.Columns.Add("CRDLIMIT2", GetType(System.Double))
                    dt.Columns.Item(dt.Columns.IndexOf("CRDLIMIT2")).Caption = "Πιστ/κό όριο 2"
                    ...
                    For Each dd In months
                        ...
                        dt.Columns.Add(String.Format("{0:00}", dd.Period) & "_" & dd.FiscPrd & " - ΦΠΑ", GetType(System.Decimal))
                        dt.Columns.Item(dt.Columns.IndexOf(String.Format("{0:00}", dd.Period) & "_" & dd.FiscPrd & " - ΦΠΑ")).Caption = String.Format("{0:00}", dd.Period) & "/" & dd.FiscPrd & " - ΦΠΑ"
                    Next
                    ...
                    If trdrs.Count < 2100 Then
                        trdrtbl = db.TRDRs.Where(Function(f) trdrs.Contains(f.TRDR)).ToList
                    Else
                        For Each tr In trdrs
                            Dim td1 = db.TRDRs.Where(Function(f) f.TRDR = tr).FirstOrDefault
                            trdrtbl.Add(td1)
                        Next
                    End If
                        ...
                        Try
                            For Each mm In months
                                ...
                                If Not IsNothing(trdb) Then
                                    mm.LDebit = trdb.LDEBIT
                                    mm.LTurnOvr = trdb.LTURNOVR
                                End If
                            Next
                            ...
                            If Not IsNothing(trdr.SALESMAN) Then
                                ...
                                End If
                            End If
                            dr("CRDLIMIT1") = If(trdr.CRDLIMIT1, 0)
                            dr("CRDLIMIT2") = If(trdr.CRDLIMIT2, 0)
                            If Not IsNothing(trdr.TRDEXTRA) Then
                                dr("NUM01") = If(trdr.TRDEXTRA.NUM01, 0) 'Ασφ/κό όριο
                            End If

                            For Each dd In months
                                dr(String.Format("{0:00}", dd.Period) & "_" & dd.FiscPrd) = dd.LDebit
                                dr(String.Format("{0:00}", dd.Period) & "_" & dd.FiscPrd & " - ΦΠΑ") = dd.LTurnOvr
                            Next
                            dt.Rows.Add(dr)
                        Catch ex As Exception
                            MsgBox(ex.Message)
                    ...
                    For Each dd In months
                        ...
                        dict.Add(String.Format("{0:00}", dd.Period) & "_" & dd.FiscPrd & " - ΦΠΑ", System.Type.GetType("System.Decimal"))
                    Next
