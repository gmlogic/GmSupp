...
#Region "001-Declare Propertys"
    Private drvValue As ccCRouting
    'Dim MTRL_S As List(Of Panel.MTRL)
    Public Property CurDrv() As ccCRouting
        Get
            Return drvValue
        End Get
        Set(ByVal value As ccCRouting)
            drvValue = value
        End Set
    End Property
    ...
    Public Property DgdvRefresh() As Boolean
        ...
        End Set
    End Property
    Private newToTransformValue As Integer
    Public Property ToTransform() As Integer
        Get
            Return newToTransformValue
        End Get
        Set(ByVal value As Integer)
            newToTransformValue = value
        End Set
    End Property
    'Private newCFINDOC_MTRLINEValue As List(Of FINDOC_MTRLINE)
    'Public Property CFINDOC_MTRLINE() As List(Of FINDOC_MTRLINE)
    '    Get
    '        Return newCFINDOC_MTRLINEValue
    '    End Get
    '    Set(ByVal value As List(Of FINDOC_MTRLINE))
    '        newCFINDOC_MTRLINEValue = value
    '    End Set
    'End Property
    Private newSeriesValue As Integer
    Public Property NSeries() As Integer
        Get
            Return newSeriesValue
        End Get
        Set(ByVal value As Integer)
            newSeriesValue = value
        End Set
    End Property
    Private newSOSOURCEValue As Integer
    Public Property NSOSOURCE() As Integer
        Get
            Return newSOSOURCEValue
        End Get
        Set(ByVal value As Integer)
            newSOSOURCEValue = value
        End Set
    End Property
    Private newPosValue As Integer
    Public Property P_ccCRouting() As Integer
        Get
            Return newPosValue
        End Get
        Set(ByVal value As Integer)
...
#Region "02-Save Data"
    ...
    Private Function Conditions() As Boolean
        ...
        Dim drv As ccCRouting = Me.MasterBindingSource.Current
...
#Region "98-Local Methods"
    Friend Sub Cmd_Delete()
        ...
        Try
            LoadData()
            Dim f1 As ccCRouting = Me.MasterBindingSource.Current
...
#Region "99-Start-GetData"
    ...
    Private Sub LoadDataInit()
        Try
            ...
            'Me.MasterBindingSource.DataSource = InitQuery
    ...
    Private Sub LoadData()
        Try
            ...
            'Dim emptyMTRL As Panel.MTRL() = Nothing
            'Dim q1 = Nothing
            'Dim f1 As Panel.FINDOC = Me.MasterBindingSource.Current
            'Select Case f1.SERIES
            '    Case 9520, 9590 'Apografi

            '        emptyMTRL = {New Panel.MTRL With {.CODE = "<Select a product>", .MTRL = 0}}
            '        q1 = (From Empty In emptyMTRL).Union(
            '                                    From m In db.MTRLs Where m.MTRCATEGORY = 7
            '                                    Order By m.CODE)
            '        Dim GetMTRL As List(Of Integer) = f1.ccCRouting.Select(Function(f) f.MTRL).ToList

            '        Dim q2 = (From m In db.MTRLs Where GetMTRL.Contains(m.MTRL)
            '                  Order By m.CODE).ToList
            '        MTRL_S = New List(Of Panel.MTRL)
            '        If q2.Count > 0 Then
            '            MTRL_S.AddRange(q2)
            '        End If
            'End Select
            Dim emptyCity As ccCTrdBCity() = Nothing
            Dim q1 = Nothing
            emptyCity = {New ccCTrdBCity With {.CITY = "<Επιλέγξτε>", .ccCTrdBCity = 0}}
            q1 = (From Empty In emptyCity).Union(
                                                From ci In db.ccCTrdBCities Where ci.COUNTRY = 1000
                                                Order By ci.CITY)

            Me.CcCTrdBCityBindingSource.DataSource = q1
            db.Log = Console.Out
            'Me.MTRLBindingSource.DataSource = q1

            'Me.MasterDataGridView.DataSource = Me.CcCRoutingBindingSource
    ...
    Private Sub MasterBindingSource_AddingNew(sender As Object, e As System.ComponentModel.AddingNewEventArgs) Handles MasterBindingSource.AddingNew
        ...
        End Try
    End Sub
    Private Function ZeroFindoc(NFINDOC As ccCRouting) As ccCRouting
        Try
            'NFINDOC.COMPANY = 1
            'NFINDOC.LOCKID = 1
            ''NFINDOC.FINDOC = 0
