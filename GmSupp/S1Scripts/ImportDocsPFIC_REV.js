...
Connect DBDriver DocData   {
    ...
    connect ('XADODrv.bpl', 'MSSQL', '192.168.140.201\sqlexpress', 'gm', '1mgergm++', 'S1REVERA');   
    ...
    and m.mtrl not in (select mtrl from cccMultiCompData where CompanyT=1002);
    ...
    sItelines=SELECT      c.CompanyT,  m.MTRL, c.CodeExp, m.LINEVAL, mt.CODE AS mcode,
                             (SELECT        CostCenterComp
                               FROM            cccMultiCompCC
                               WHERE        (CostCenterCode = d.Code) AND (CompanyT = 1002)) AS BU
...
WHERE        (f.FINDOC = :sSupDoc.findoc) AND (c.CompanyT = 1002);
...

{
    ...
    }else
    {
        ...
        fetch sCheck
        {
            x = ExecSql('DocData','insert into cccMultiCompData (mtrl,companyt) values ('+VarToStr(sCheck.mtrl)+',1002)',null);
