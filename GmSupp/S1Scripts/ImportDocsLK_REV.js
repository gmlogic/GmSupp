...
var
    vTmp, vSeries, vBranch, xdebug, vMtrl, vTrdr, vCompanyT;
vCompanyT = 2001;//LK 1001 LNK
...
Connect DBDriver DocData   {
    ...
    //trdr=38236 //25353
    //trdr=39417 ΚΩΔ.00020 ΛΙΠΑΣΜΑΤΑ ΚΑΒΑΛΑΣ ΛΤΔ
    sCheck = select DISTINCT m.MTRL, mt.code, mt.name, 2001 as CompanyT
    FROM FINDOC AS f
    INNER JOIN MTRLINES AS m ON f.FINDOC = m.FINDOC
    INNER JOIN MTRL AS mt ON m.MTRL = mt.MTRL
    LEFT OUTER JOIN cccTrdDep AS d ON m.cccTrdDep = d.cccTrdDep
    where f.trdr = 39417
    and f.sosource = 1351
    and f.tfprms in (102, 103)
    and f.trndate >=: ImpTable.vFromdate
    and f.trndate <=: ImpTable.vToDate
    and m.mtrl not in (select mtrl from cccMultiCompData where CompanyT = 2001);
    ...
    FROM            cccMultiCompCC
    WHERE(CostCenterCode = d.Code) AND(CompanyT = 2001)) AS BU
    ...
    WHERE(f.FINDOC = : sSupDoc.findoc) AND(c.CompanyT = 2001);
...

{
    ...
    }else
    {
        ...
        fetch sCheck
        {
            x = ExecSql('DocData', 'insert into cccMultiCompData (mtrl,companyt) values (' + VarToStr(sCheck.mtrl) + ',' + VarToStr(sCheck.CompanyT) + ')', null);
