                ...
                while (!ITELINES.EOF) {
                    ...
                    if (vGroupList == 1) {
                        ...
                        } else //if(sg.mtrgroup==rset.GroupYD)
                        {
                            swh = X.GETSQLDATASET('select whouse from mtrdoc where findoc=' + ITELINES.FINDOCS, null);
                            vWhouse = swh.whouse;
                            sp = X.GETSQLDATASET('select dbo.fn_clDiscStep3(' + vDate + ',' + vWhouse + ',' + SALDOC.TRDBRANCH + ',' + ITELINES.MTRL + ',' + SALDOC.TRDR + ',' + SALDOC.TRDR_CUSTOMER_TRDBUSINESS + ',' + SALDOC.SHIPMENT + ',' + '\'' + MTRDOC.QTY1 + '\'' + ') AS dsc', null);
